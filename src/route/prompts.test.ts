...
test("production system prompt contains no executable legacy GitHub recipes", () => {
  ...
  expect(systemPrompt).toContain("Delegated child threads are non-interactive too")
...
test("ci_fix result sticky instructions produce a schema-valid concrete key", () => {
  ...
  expect(example?.[1]).toBe("cifix:pr-123")
  expect(example?.[1]).toMatch(/^[a-z0-9][a-z0-9:._-]{0,63}$/)
  expect(systemPrompt).not.toContain("cifix:pr-$BOT_PR_NUMBER")
  expect(systemPrompt).toContain("Never pass `$`, a variable")
  expect(systemPrompt).toContain("name, or angle brackets in the key")
})
